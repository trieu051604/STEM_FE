...
export function MasterAdminDashboard() {
  ...
  if (loading) {
    return (
      <div className="bg-slate-950 text-slate-100 min-h-screen p-6 rounded-3xl border border-slate-900 shadow-2xl flex flex-col items-center justify-center py-20">
    ...
    <div className="bg-slate-950 text-slate-100 min-h-screen p-6 rounded-3xl border border-slate-900 shadow-2xl space-y-6 relative pb-20 font-sans selection:bg-blue-500/30">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/60 pb-5">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border border-slate-800 bg-slate-900 text-slate-300 hover:bg-slate-800 hover:text-white transition-all"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="w-12 h-12 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-450 border border-blue-500/20">
        ...
        >
          <div className="w-12 h-12 rounded-xl bg-amber-500/10 flex items-center justify-center text-amber-450 border border-amber-500/25">
            <ClipboardCheck className="w-6 h-6 text-amber-450 text-amber-400" />
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Yêu cầu chờ duyệt</p>
            <h3 className="text-3xl font-extrabold mt-0.5 text-amber-400">
        ...
        >
          ...
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Tổng người dùng</p>
        ...
        >
          ...
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Khóa học hệ thống</p>
            <h3 className="text-3xl font-extrabold mt-0.5 text-white">
      ...
      {/* Quick Actions */}
      <div className="bg-slate-900/30 backdrop-blur-md border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
        <h2 className="text-lg font-bold text-white font-headline">Thao tác nhanh</h2>
        <div className="flex flex-wrap gap-3">
          <Link to="/dashboard/schools">
            <Button variant="outline" className="gap-2 border-slate-800 bg-slate-950 text-slate-300 hover:bg-slate-800 hover:text-white">
      ...
      <div className="bg-slate-900/30 backdrop-blur-md border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
        ...
        <div className="space-y-3">
          {activities.length === 0 ? (
            <div className="py-8 text-center text-slate-500 border border-dashed border-slate-850 rounded-xl">
              <Activity className="w-8 h-8 mx-auto mb-2 opacity-25 text-blue-400" />
            ...
            activities.map((activity) => (
              <div
                key={activity.id}
                className="flex items-start gap-4 p-4 rounded-xl hover:bg-blue-500/[0.01] border border-transparent hover:border-slate-800/40 transition-all"
              >
                <div className="w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-450 border border-blue-500/20 shrink-0">
                  <Icon
      ...
      {stats && stats.pendingSchoolRequests && stats.pendingSchoolRequests > 0 && (
        <div className="bg-amber-500/10 border border-amber-500/25 rounded-2xl p-4 flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-amber-500/15 flex items-center justify-center shrink-0 border border-amber-500/20">
            <AlertCircle className="w-5 h-5 text-amber-455 text-amber-400" />
          </div>
